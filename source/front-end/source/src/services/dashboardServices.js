...
const buildGraph = (response, title) => {
  ...
  let trend;
  if (trendValues > 5) trend = "up";
  else if (trendValues < -4) trend = "down";
  else trend = "neutral";
...
export const summarizeRequests = async (departmentId) => {
  ...
  try {
    ...
    const lowStock = response.filter((r) => r.materialStatus === "LowStock");
    const alert = response.filter((r) => r.materialStatus === "Damaged");
    if (!Array.isArray(response) || response.length === 0) return defaultResult;

    return [
      buildGraph(response, "Total Requests"),
      buildGraph(lowStock, "Low Stock Requests"),
      buildGraph(alert, "Damaged Requests"),
    ];
  } catch (err) {
