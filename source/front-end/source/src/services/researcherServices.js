...
export const searchAvailableMaterials = async (departmentId, searchTerm) => {
  return await apiCall(
    `/researcher/${departmentId}/material/search?q=${encodeURIComponent(
      searchTerm
    )}`
  );
};
...
export const useMaterial = async (materialId, quantity_used) => {
  ...
  const params = new URLSearchParams({
    quantity_used: String(Math.floor(Number(quantity_used))),
  });

  const res = await apiCall(
    `/material/${materialId}/use?quantity_used=${quantity_used}`,
    {
      method: "POST",
    }
  );
